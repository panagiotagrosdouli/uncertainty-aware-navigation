...
def main() -> None:
    ...
    for seed in seeds:
        ...
        for planner_name in planning_config["planners"]:
            ...
            for lambda_uncertainty in lambda_values:
                ...
                if seed == seeds[0] and (
                    planner_name == "astar" or lambda_uncertainty == max(planning_config["lambda_uncertainty"])
                ):
                    figure_name = f"{planner_name}_lambda_{lambda_uncertainty}_executed_path.png"
                    plot_map_with_path(
                        environment.occupancy,
                        result.path_history,
                        figures_dir / figure_name,
                        title=f"Executed path: {planner_name}, lambda={lambda_uncertainty}",
                    )
    ...
    summary = summarize_results(
        ...
        metrics=["steps", "replans", "collisions", "final_coverage", "final_mean_uncertainty"],
